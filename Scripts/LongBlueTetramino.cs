...
public class LongBlueTetramino : MonoBehaviour
{
    ...
    private float verticalStep = -1.5f, fallSpeed = 1f, fallTimer = 0, sideSpeed = .1f, sideTimer, rightKeyWaitTimer, leftKeyWaitTimer;
        ...
        foreach(Transform children in this.transform) {
            ...
            foreach(var hitCollider in hitColliders)
            {
                if(hitCollider.gameObject.name != this.gameObject.name 
                    ...
                    && hitCollider.gameObject.name != "BlueTetramino(Clone)") {
                    return false;
                }
            }
        }
